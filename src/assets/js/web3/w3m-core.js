 ...
 */
 ...
 // Unpkg imports
// import * as dxwAbi from "../dxwAbi.ts";
const Web3Modal = window.Web3Modal.default;
const WalletConnectProvider = window.WalletConnectProvider.default;
const Fortmatic = window.Fortmatic;
const evmChains = window.evmChains;
const DXWContractAddress = "0x89675dccfe0c19bca178a0e0384bd8e273a45cba";
...
async function fetchAccountData() {
  ...
  const rowResolvers = accounts.map(async (address) => {
    ...
    // dxw balance
    let contract = new web3.eth.Contract(getABI(), DXWContractAddress);
    const dxwBalance = await contract.methods.balanceOf(address).call();
    const humanFriendlyDxwBalance = parseFloat(dxwBalance / 1000000000000000000).toFixed(4);
    ...
    clone.querySelector(".dxwBalance").textContent = humanFriendlyDxwBalance;
...
window.addEventListener('load', async () => {
  ...
  document.querySelector("#btn-reduce").addEventListener("click", reduceDialog);
});
